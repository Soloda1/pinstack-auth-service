.PHONY: test test-unit test-integration test-auth-integration clean build run docker-build setup-system-tests setup-monitoring start-monitoring start-prometheus-stack start-elk-stack stop-monitoring clean-monitoring check-monitoring-health logs-prometheus logs-grafana logs-loki logs-elasticsearch logs-kibana start-dev-full stop-dev-full clean-dev-full start-dev-light quick-test-local
...
# Интеграционные тесты только для auth service
test-auth-integration: check-services
	@echo "🧪 Запуск интеграционных тестов для Auth Service..."
	cd $(SYSTEM_TESTS_DIR) && \
	go test -v -count=1 -timeout=10m ./internal/scenarios/integration/gateway_auth/...

# Остановка всех контейнеров
stop-auth-infrastructure:
...
test-integration: start-auth-infrastructure test-auth-integration stop-auth-infrastructure
...
# Очистка
clean: clean-auth-infrastructure
	go clean
	rm -f $(BINARY_NAME)
...
start-dev-full: setup-monitoring start-monitoring start-auth-infrastructure
...
stop-dev-full: stop-monitoring stop-auth-infrastructure
	@echo "🛑 Полная dev среда остановлена"

# Очистка всей dev среды
clean-dev-full: clean-monitoring clean-auth-infrastructure
	@echo "🧹 Полная dev среда очищена"

# Запуск только с Prometheus stack (без ELK)
start-dev-light: setup-monitoring start-prometheus-stack start-auth-infrastructure
